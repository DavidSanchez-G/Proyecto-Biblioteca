...
<head>
    ...
    <style>
        ...
        .info-busqueda { text-align: center; margin-top: 140px; margin-bottom: -15px; color: #602b06; font-size: 1.1rem; }
        .sub-info { font-size: 0.75rem; color: #666; display: block; margin-top: 2px; }
...
<body>
        ...
        <ul>
            ...
            <li class="botones-nav">
                <form action="ver-prestamos.php" method="GET">
                    <input type="text" name="busqueda" value="<?php echo htmlspecialchars($busqueda); ?>" placeholder="Nombre, Título, ISBN o Carnet..." required>
                    <button>🔍</button>
                </form>
            </li>
        </ul>
    ...
    <article class="vista-lista" style="display: block; margin-top: 20px;">
        <table>
            ...
            <tbody>
    <?php
        // --- 3. CONSULTA SQL DINÁMICA ACTUALIZADA ---
        $sql = "SELECT 
                    p.id_prestamo, 
                    a.nombre, 
                    a.apellidos, 
        ...
        // Se añade ISBN y Carnet a la búsqueda
        if (!empty($busqueda)) {
        ...
        function aplicarFiltros() {
            ...
            document.querySelectorAll('.vista-lista tbody tr').forEach(fila => {
                ...
                if (estadoSeleccionado.length > 0 && estadoSpan) {
                    mostrar = estadoSeleccionado.includes(estadoSpan.textContent.trim());
                }
